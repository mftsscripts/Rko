...
async def sentinel(ctx):
    await ctx.send(f"Jsem online a připraven posloužit !")
...
async def dm(ctx, member: discord.Member, *, msg):
    try:
        await ctx.send(f"📝 Posílám zprávu uživateli {member.mention}")
        await log_to_channel(f"📨 **{ctx.author}** poslal dm uživateli **{member}** v {ctx.channel.mention}")
    except discord.Forbidden:
        await ctx.send(f"❌ Nepodařilo se odeslat dm uživateli {member.mention}.")
        await log_to_channel(f"⚠️ **{ctx.author}** se pokusil napsat dm uživateli **{member}**, ale selhal.")
